...
public struct UploaderVisitRecord: Identifiable, Codable, Hashable {
    ...
    public init(id: Int, name: String, avatarURL: URL?, signature: String, visitedAt: Date) {
        self.id = id
        self.name = name
...
public final class UploaderVisitStore: ObservableObject {
    @Published public private(set) var records: [UploaderVisitRecord] = []

    private let fileURL: URL
    private let queue = DispatchQueue(label: "uploader.visit.store.queue", qos: .userInitiated)

    public init(filename: String = "uploader_visit_history.json") {
    ...
    public func save(profile: UploaderProfile) {
