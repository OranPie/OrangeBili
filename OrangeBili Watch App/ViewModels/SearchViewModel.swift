...
final class SearchViewModel: ObservableObject {
    ...
    @Published var order = "totalrank"
    @Published var scope: Scope = .video
    @Published private(set) var videoResults: [BiliVideo] = []
    @Published private(set) var userResults: [UserSearchResult] = []
    @Published private(set) var articleResults: [ArticleSearchResult] = []
    ...
    func loadMore() async {
        ...
        do {
            ...
            DebugLogStore.shared.log(
                category: "search",
                message: "viewmodel fail scope=\(scope.rawValue) keyword=\(trimmed) page=\(page) order=\(order.isEmpty ? "default" : order) err=\(error.localizedDescription)"
            )
